...
## Overview
The lab focused on deploying managed disks in Azure, and I covered:
...
## Business Scenario
My team wants to automate and simplify Azure resource deployments. By using templates, I can reduce administrative overhead, minimize human error, and increase consistency across deployments.
...
1. **Create a Resource Group** – I started by creating the resource group for the lab.  
   ![1.1](screenshots/1.1.png)

2. **Create a managed disk** – I went to **Disks** and created a new 32GB Standard HDD disk.  
   ![1.2](screenshots/1.2.png)

3. **Review and create** the disk.  
   ![1.3](screenshots/1.3.png)

4. **Export the ARM template** – after deployment, I went to **Automation → Export template**, reviewed it, and clicked **Download**.  
   ![1.4](screenshots/1.4.png)

5. **Template files** – I extracted the ZIP, which contained `template.json` and `parameters.json`, for the next deployment.  
   ![1.5](screenshots/1.5.png)

---

### Task 2 – Deploy a new managed disk using a custom template
1. I opened **Deploy a custom template** in the portal and uploaded the downloaded template.  
   ![2.1](screenshots/2.1.png)

2. I edited the template:
   - Changed the default value of `disk_name` to `"az104-disk2"`
   - Updated the parameter name to `"disk_name"`  
   ![2.2](screenshots/2.2.png)

3. I edited `parameters.json` to match the template parameter name.  
   ![2.3](screenshots/2.3.png)

4. I deployed the resource.  
   ![2.4](screenshots/2.4.png)

5. I verified that the new disk appeared in the Azure portal.  
   ![2.5](screenshots/2.5.png)

6. I checked the deployment under **Resource Group → Deployments**.  
...
1. **Configure Cloud Shell storage** – before accessing PowerShell, I created a new storage account to store the files I would need.  
   ![3.1](screenshots/3.1.png)

2. **Upload template files** – in PowerShell Cloud Shell, I switched to Classic view and uploaded `template.json` and `parameters.json`.  
   ![3.2](screenshots/3.2.png)

3. **Edit the template** – I updated the default value of `disk_name` in lines 5–6.  
   ![3.3](screenshots/3.3.png)

4. **Edit the parameters file** – I modified line 5 to match the template.  
   ![3.5](screenshots/3.5.png)

5. **Deploy the template** using:
   ```powershell
...
7. **Switch to Bash** in Cloud Shell and edit `template.json` and `parameters.json`, now naming the disk `"disk4"`.
...
2. I verified the disk was created successfully:
...
1. I uploaded `azuredeploydisk.bicep` to Cloud Shell and edited:
...
2. I deployed the Bicep template:
...
3. I verified the disk creation:
...
To avoid unnecessary costs, I deleted the resource group, which removes all associated resources:
